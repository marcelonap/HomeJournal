...
class ListItemsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create list_item" do
    assert_difference("ListItem.count") do
      post list_items_url, params: { list_item: { name: @list_item.name, qty: @list_item.qty, status: @list_item.status, store: @list_item.store } }
    end
  ...
  test "should update list_item" do
    patch list_item_url(@list_item), params: { list_item: { name: @list_item.name, qty: @list_item.qty, status: @list_item.status, store: @list_item.store } }
